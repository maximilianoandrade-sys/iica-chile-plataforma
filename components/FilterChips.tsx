  ...
  return (
    <section className="bg-white dark:bg-gray-900 border-b border-[var(--iica-border)] dark:border-gray-700 sticky top-[var(--header-height)] z-40">
      <div className="container mx-auto max-w-[1200px] px-4 py-4">
        ...
        {/* ── Quick-access chips ── */}
        <div className={`space-y-2 transition-opacity duration-200 ${isPending ? 'opacity-70' : ''}`}>

          {/* Fila 1: VER */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 w-8 shrink-0 select-none">
              Ver
            </span>
            <QuickChip
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <ChevronDown className={`w-3.5 h-3.5 transition-transform hidden sm:block ${advancedOpen ? 'rotate-180' : ''}`} />
            </button>
          </div>

        </div>

        {/* ── Active filter chips + feedback de búsqueda ── */}
        <div className="flex flex-wrap items-center justify-between gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-800" aria-live="polite">
          <div className="flex flex-wrap items-center gap-2">
            {hasActiveFilters && (
              <>
          ...
          {/* Grupo 2: Feedback estado búsqueda en tiempo real */}
          {isPending ? (
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-[var(--iica-blue)] animate-pulse">
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
              Buscando…
            </span>
          ) : hasActiveFilters ? (
            <span className="text-xs text-gray-400">↓ Resultados abajo</span>
          ) : null}
        </div>

        {/* ── Panel avanzado DESKTOP (inline, solo sm+) ── */}
        {advancedOpen && (
          <div
            id="advanced-filters-panel"
            className="hidden sm:block mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 animate-in slide-in-from-top-2 duration-200"
          >
            <AdvancedFiltersContent
              ...
              normalizeAmountRange={normalizeAmountRange}
      ...
      {/* ── Drawer MOBILE (bottom sheet, solo < sm) ── */}
      {advancedOpen && typeof window !== 'undefined' && createPortal(
        <>
          <div
          ...
          <div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 sm:hidden bg-white dark:bg-gray-900 rounded-t-2xl shadow-2xl max-h-[85dvh] overflow-y-auto animate-in slide-in-from-bottom duration-300"
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
              ...
              <button
                ...
                className="rounded-full p-2 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
              >
            ...
            <div className="sticky bottom-0 px-4 py-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
              <button
                type="button"
                onClick={() => setAdvancedOpen(false)}
                className="w-full rounded-xl bg-[var(--iica-blue)] text-white font-semibold py-3 min-h-[48px] text-base active:opacity-80 transition-opacity"
              >
                {advancedFilterCount > 0
                  ? `Ver resultados (${advancedFilterCount} filtro${advancedFilterCount !== 1 ? 's' : ''} activo${advancedFilterCount !== 1 ? 's' : ''})`
                  : 'Ver resultados'}
              </button>
            </div>
          </div>
...
/* ── Advanced Filters Content sub-component (shared: desktop panel + mobile drawer) ── */
interface AdvancedFiltersContentProps {
  ...
  updateParams: (updates: Record<string, string>, mode?: 'push' | 'replace') => void;
  normalizeDateRange: (from: string, till: string) => { postedFrom: string; postedTill: string };
  normalizeAmountRange: (min: string, max: string) => { minAmount: string; maxAmount: string };
  stringifyCsv: (values: string[]) => string;
}
