/**
 * Scraper MINAGRI — Licitaciones en MercadoPublico
 *
 * Usa el feed RSS/HTML del portal MercadoPublico para la organización
 * del Ministerio de Agricultura de Chile (MINAGRI). Mismo patrón que
 * fia-licitaciones: selector `a.Link` con fallback a `a` con patrón "Id: XXXX".
 */
...
const FEED_URL =
  "https://www.mercadopublico.cl/Portal/FeedOrg.aspx?qs=Mer+eNmLUHn7TN5dx6I0Mg==";
...
export const minagriFeedScraper: Scraper = {
  slug: "minagri-licitaciones",
  name: "MINAGRI — Licitaciones MercadoPublico",
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    // Selector primario; fallback por patrón textual "Id: XXXX"
    ...
    candidates.each((_, element) => {
      ...
      projects.push({
        title: `[MINAGRI Licitación] ${title}`,
        institution: "MINAGRI",
        url: detailUrl,
        canonicalKey: detailUrl,
        deadline: parseMercadoPublicoDate(fechaRaw),
        description: descrRaw || "Licitación MINAGRI en MercadoPublico.",
        ambito: "Nacional",
        opportunityType: "Licitacion",
        tags: ["MINAGRI", "Mercado Publico", "Licitacion"],
      });
    });
