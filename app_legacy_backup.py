...
# Definición del decorador de caché
def cache_proyectos(max_age=1800):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            now = time.time()
            if 'result' in cache:
                timestamp, result = cache['result']
                if now - timestamp < max_age:
                    return result
            # Si no hay caché válido, ejecutar función
            try:
                result = func(*args, **kwargs)
                cache['result'] = (now, result)
                return result
            except Exception as e:
                print(f"Error en función decorada: {e}")
                # Si fallamos y tenemos caché viejo, devolverlo como fallback
                if 'result' in cache:
...
# Importa todos los scrapers (Importación dinámica o manual requerida aquí si no están)
app = Flask(__name__)
ANALYTICS_AVAILABLE = False  # Definir variable global para evitar NameError
...
def home():
    ...
    response = make_response(render_template("home_actualizado.html",
