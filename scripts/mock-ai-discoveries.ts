 ...
 *
 * Inserta 6 proyectos simulando lo que Claude devolvería en un scan semanal,
 * con discoveredBy='ai' y needsReview=true. Los snippets son ejemplos de
 * lo que pondría Claude (citas textuales de search results).
 *
 * Para borrarlos: DELETE FROM "Project" WHERE "canonicalUrl" LIKE 'https://mock-discovery%';
 *
 * Para reemplazar con discovery real: cargá ANTHROPIC_API_KEY con créditos
 * y corré scripts/discover-projects.ts.
 */
  ...
  for (const m of MOCK_DISCOVERIES) {
    ...
    await prisma.project.create({
      data: {
        ...
        notasInternas: `AI snippet: "${m.snippet}"`,
        discoveredBy: "ai",
        needsReview: true,
